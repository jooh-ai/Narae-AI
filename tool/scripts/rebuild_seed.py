...
무엇을 고치는가 (근거가 확정된 것만)
  ...
  4) 2025-04-15 제외 — 증설(+5.56MW) 전 시험으로 판단 (EXCLUDE 주석 참조).
...
#
# 2025-04-15 는 설비 구성이 다른 시험으로 판단된다:
#   · 보정값 +1.788 vs 인접(12~15°C) 평균 +6.320 → -4.53 MW 어긋남
#   · +5.56 을 더하면 +7.348 로, 2025-11-04(CIT 14.4)의 +7.351 과 사실상 동일
#   · 실적표 '연도' 열에 'IGV' / 'IGV (+5.56MW)' 두 그룹 라벨이 있다
#   · 정정 전 씨앗에도 이 건은 없었다(담당자도 넣지 않았던 것으로 보인다)
#   · 넣으면 GP LOOCV MAE 가 1.2379 → 1.4321 로 나빠지고, 이 건의 LOOCV 오차가
#     4.223 MW 로 32건 중 최대다
# 증설 전 시험을 섞으면 보정곡선이 통째로 5MW 가량 끌려 내려간다. 담당자에게
# 'IGV (+5.56MW)' 라벨의 적용 구간을 확인한 뒤, 필요하면 add 명령으로 넣는다.
EXCLUDE = {
    "2025-04-15": "증설(+5.56MW) 전 시험으로 판단 — 담당자 확인 후 add 로 추가",
}
