...
해결 (2026-08-25, 담당자 확인)
    담당자가 엑셀2 곡선엔진과 엑셀4 이론값을 두 날짜에 대해 직접 대조했고,
    **엑셀2 AB6(일괄 보정값 BLT)를 0 으로 두면 엑셀4 이론값과 같았다.**
    엑셀2 O열의 조건은 「그 날 대기압을 온도별 일괄 적용 + RH 60% 고정」이다.

    → **곡선은 낡지 않았다.** 엑셀2(살아있는 엔진) = 엑셀4(동결) = 우리 base_table.
      아래 ④ 에서 걱정했던 저온 4.6 MW 는 근거가 없어졌다. base_table 교체 불필요.

    엑셀4 에는 이론값 성격의 컬럼이 두 개 있고 습도 취급이 다르다 — 헷갈리기 쉽다.
      · '온도 Profile' 시트      : RH 60 고정  = 엑셀2 O열(BLT=0) = 우리 base_table
      · '실측데이터' 시트 이론기준값 : **실측 RH**  = 우리 theory_cc(rh=실측)
    담당자가 대조한 것은 앞쪽이다. 뒤쪽이 실측 RH 임은 전사 25건으로 확정된다:
        실측 RH 반영 → 평균오차 0.055 / 최대 0.190 MW
        RH 60 고정   → 평균오차 0.808 / 최대 2.634 MW
    `보정값 = CC실측 − 이론기준값 − W` 의 기준선은 뒤쪽이고, 우리는 그쪽을 쓴다.

    남은 미해결 — Base load 실적표의 '공급가능용량' 컬럼이 무엇인지는 여전히 모른다.
      · BLT 를 적용해도 설명되지 않는다 (2025 상반기 16건, Δ vs BLT r=+0.023).
        BLT 를 빼면 오히려 나빠진다 (sd 1.64 → 2.62). 그 컬럼은 BLT=0 이다.
      · 진공 항으로도 설명되지 않는다. Δ ~ CIT + 진공도 다중회귀의 진공 계수는
        -0.069 MW/mbar 로 물리 예상(-0.17~-0.25)의 1/3 이고, 역산되는 설계 진공
        곡선이 CIT 0°C 에서 5.5 mbar — 물리적으로 불가능하다(0°C 포화압 6.1 mbar).
      · 입찰에 쓰지 않는 사후 분석 컬럼이므로 더 파지 않는다. 필요해지면
        담당자에게 그 셀 수식을 받는 것이 유일한 확실한 방법이다.

관측 (2026-08-24, 실적표 58건 전수 대조)
...
우리 Tool 은 ①을 쓰는 게 맞다 — 그리고 곡선 차이는 최종 입찰값에서 상쇄된다
    ...
    ※ 이 ④ 는 "담당자 곡선이 최신일 수도 있다" 는 가정 아래 계산한 최악값이다.
      2026-08-25 담당자 확인으로 그 가정이 깨졌으므로(엑셀2 = 엑셀4 = 우리)
      실제 위험은 아니다. 기준선을 바꿀 이유가 생겼을 때 다시 쓰는 계산이다.
      다만 저온 시험이 3건뿐이라는 사실 자체는 그대로 남는 약점이다.
