...
R² 를 조심할 것
    R² = 1 − SSE/SST 이고 SST 는 '실측값이 평균에서 흩어진 정도'다. 그래서
      ...
      · 보정값 기준 R² 는 실측 보정값의 산포에 따라 뜻이 달라진다. 거의 일정하면
        SST 가 0 에 가까워 예측이 조금만 흔들려도 R² 가 무너진다 — 모델이 나쁜
        게 아니라 그 표본으로 R² 를 쓸 수 없다는 뜻이다. 산포가 커지면 읽을 수
        있게 된다. 그래서 화면 주석은 **실제 sd 를 보고** 달라진다(sd 0.5 MW 기준).

        실제로 겪은 변화: 3회차까지는 실측 보정값이 -0.700/-0.579/-0.620 으로
        sd 0.062 MW 였고 GP R² = -521 이었다. 4회차(2026-07-22, -4.855)가 들어오자
        sd 2.112 MW 가 되고 GP R² = +0.262 로 처음 해석 가능한 값이 됐다.

    n<10 에서는 R² 를 단독 근거로 쓰지 않는다. MAE·RMSE·편차·미달과 함께 본다.
...
R² 대신 무엇을 보는가 — 스킬 점수
    ...
    4회차 결과: GP +0.913 · 커널 +0.873 · 구간평균 +0.863
    (= 일괄 대비 오차제곱을 각각 91.3% · 87.3% · 86.3% 줄였다)
...
def main() -> None:
    ...
    corrs = [p["R"]["bin"].meas_corr for p in per]
    mu = sum(corrs) / len(corrs)
    sd = ((sum((c - mu) ** 2 for c in corrs) / (len(corrs) - 1)) ** 0.5
          if len(corrs) > 1 else float("nan"))
    print(f"\n   ※ 실측 보정값 산포 sd {sd:.3f} MW (평균 {mu:+.3f})")
    if sd < 0.5:
        # 표본이 뭉쳐 있으면 R² 의 분모(SST)가 0 에 가까워 지표가 무의미해진다.
        print("     이 표본은 거의 일정하다 → 보정값 기준 R² 는 쓸 수 없다.")
        print("     크게 음수로 나오는 것은 모델이 나빠서가 아니라 SST 가 0 에")
        print("     가까워서다. 산포가 커질 때까지 MAE·편차·미달로 판단한다.")
    else:
        print("     산포가 충분해져 보정값 기준 R² 를 읽을 수 있다 —")
        print("     '평균 찍기' 대비 성적이다. 음수면 평균보다 못하다는 뜻이다.")
    print("     Net 기준 R² 는 여전히 참고만 한다 — 온도로 생기는 큰 분산은")
    print("     이론곡선이 설명하는 몫이고 모델의 공헌이 아니다.")
    print("     스킬은 분모가 '종전 일괄 보정' 이라 표본이 작아도 해석이 산다.")
    if len(per) < 10:
