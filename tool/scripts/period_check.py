"""2025 상반기 실적 대조 — 시기 차이인가, IGV Turn-up 미실시인가.

결론 (2026-08-24 정정)
    저온·중온대(CIT<20)는 **IGV Turn-up 미실시**로 전부 설명된다. 시기 차이가 아니다.
    처음에는 "2025 상반기는 성능 수준이 다른 시기(-4.09 MW)"로 판단했는데, 그 -4.09 가
    바로 W(IGV)=+4 였다. 2026-08 시운전에서 8건 중 3건이 IGV 미실시였고 그 3건이 똑같이
    4~6 MW 낮게 나온 것을 보고 되짚어 확인했다.

        보정값 = CC실측 − 이론기준값 − W(IGV)
        → 실시하지 않은 turn-up 을 빼면 보정값이 W 만큼 낮게 남는다.

    저온·중온 5건의 '내포 W'(= CC − 이론 − 모델예측)
        -1.0 · +1.8 · -0.4 · +0.2 · -0.2   →  평균 +0.09, sd 1.06
    전부 0 을 가리킨다. W=0 으로 계산하면 평균 잔차 +0.091(sd 1.063)로 곡선에 얹힌다.
    1년 뒤 2026년 같은 온도대 실적(+3.66~+7.35)과 구분되지 않는다.

    고온대(CIT>=20, 11건)는 깔끔하지 않다. 내포 W 가 +1.1~+7.7 로 흩어져(sd 2.31)
    IGV 실시 여부를 이 자료로는 판정할 수 없다. 고온 구간은 누적 DB 자체의 산포도
    5 MW 수준이라 구분이 안 된다.

확정 (2026-08-24 담당자 확인)
    IGV 실시 여부에 따라 출력 변동이 너무 커서, 일관성을 위해 IGV Turn-up 을
    실시한 시험만 보정값에 사용한다. 그러므로 이 저온·중온 5건은 **넣지 않는다**.
    W=0 으로 계산해 곡선에 얹힌다는 것이 확인됐어도, 방침상 누적 대상이 아니다.
    2025-04-15 도 영구 제외다.

    이 스크립트의 가치는 '왜 낮았는지' 를 남기는 데 있다. 시기 차이(설비 열화)로
    오판하면 곡선 전체를 잘못 끌어내리게 되므로, 다시 같은 모양의 낙차를 보면
    먼저 IGV 실시 여부를 확인해야 한다.
...
def main() -> None:
    ...
    # ── 3-b) 내포 W — 모델과 맞으려면 W 가 얼마여야 하는가 ────────────────
    # 이것이 시기 차이와 IGV 미실시를 가르는 검사다. 시기 차이라면 내포 W 가
    # 뒤죽박죽이어야 하고, IGV 미실시라면 0 에 모여야 한다.
    print("\n── 내포 W = CC실측 − 이론기준값 − 모델예측 ──")
    print("   (시기 차이라면 흩어지고, IGV 미실시라면 0 에 모인다)")
    for lo, hi, label in ((-99, 20, "저온·중온 CIT<20"), (20, 99, "고온 CIT>=20")):
        sub = [r for r in h1 if lo <= r["cit"] < hi]
        vals = [r["cc"] - r["theory"] - gp._post(r["cit"])[0] for r in sub]
        n = len(vals)
        mu = sum(vals) / n
        sd = (sum((v - mu) ** 2 for v in vals) / (n - 1)) ** 0.5 if n > 1 else float("nan")
        print(f"   {label:20s} n={n:2d}  평균 {mu:+6.2f}  sd {sd:5.2f}   "
              + " ".join(f"{v:+.1f}" for v in sorted(vals)))
    print("   → 저온·중온은 0 에 모인다(IGV 미실시). 고온은 흩어져 판정 불가.")
