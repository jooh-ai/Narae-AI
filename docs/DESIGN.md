...
5. **테스트 결과 누적 = RiMS 자동취득 + 실행자 반영 결정.** 날짜·시간 입력 → RiMS 자동취득 →
   보정값 계산. **기본은 확인용(미저장)**, `--accumulate`(GUI 체크)일 때만 누적에 반영(저장).
   → 확인용 실행이 누적을 오염시키지 않음. **같은 날짜 중복 반영 방지** 내장. 수기입력은 백필 fallback.
6. ~~**W(IGV) 출처 결정 필요.**~~ ✅ **확정(2026-06-26): 밴드값(0/2/4/6) 유지.** RiMS 취득은 17~18시
   윈도만 필요(CC 실측), W는 CIT 온도밴드(`igv_turnup`)로 산정. → 자동취득 경로 단순화.
7. ~~**CC실측 = raw vs 정규화(공급가능용량) 결정.**~~ ✅ **확정(2026-07): raw CC Load 사용.**
   이론기준값(I)이 실제조건(실측 대기압·RH)에서 산출되므로 CC실측도 실제조건 값(raw)이어야 하며,
   엑셀4 '실측데이터' CC실측(G열)이 raw(수동입력)임을 현장 확인. → OPC UA 커넥터는 raw CC Load 취득.
8. ~~**RiMS 취득 경로.**~~ ✅ **B 방식 확정·검증(2026-07): DataPARC OPC UA 직접 취득.** RiMS=DataPARC(Capstone)
   OPC UA 서버(익명 접속). CIT·대기압·GT·ST·CC 를 17~18시 TimeAvg 로 직접 취득(엑셀1/Excel 불필요).
   집계는 **서버측 TimeAverage(HistoryReadProcessed, i=2343)** — raw 수동 시간가중은 창 시작 공백+
   stale 경계값으로 왜곡됨(실사례: CC 348 vs 실제 414). A(엑셀1 경유)는 대안으로 유지.
   **병행운전 1차(과거 실테스트 2025-09-23 재현)**: CC실측 414.66(≡애드인 414.6648, 엑셀4 수기 414.54),
   이론 410.61(엑셀4 410.65), W +6(일치), 보정값 **−1.95 vs 엑셀4 −2.109 → 차이 +0.16, ±0.5 합격**.
   비핵심 태그 중 Aux Power·진공은 애드인값과 소폭 상이(계산 미사용, 중복 BrowseName 추정) — 추후 확인.
