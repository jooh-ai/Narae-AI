...
8. ~~**RiMS 취득 경로.**~~ ✅ **B 방식 확정·검증(2026-07): DataPARC OPC UA 직접 취득.** RiMS=DataPARC(Capstone)
   ...
   **병행운전 1차(과거 실테스트 2025-09-23 재현, 여름)**: CC실측 414.66(≡애드인 414.6648, 엑셀4 수기 414.54),
   이론 410.61(엑셀4 410.65), W +6(일치), 보정값 **−1.95 vs 엑셀4 −2.109 → 차이 +0.16, ±0.5 합격**.
   **병행운전 2차(2026-01-06, 겨울)**: RH 실측(23%) 반영 후 이론 **451.16 ≡ 엑셀4 451.16(완전 일치)**,
   CC실측 460.92(엑셀4 460.87), 보정값 **+5.76 vs +5.713 → 차이 +0.05, 합격**.
   **병행운전 3차(2026-02-25, 봄가을)**: CC실측 **445.60 ≡ 445.60(완전 일치)**, CIT·W 일치. 이론/보정값
   1.0MW 차이는 엑셀4가 **고장 RH 0.1%를 그대로 사용**했기 때문(rh=0.1 재현 시 잔차 0.03 — 취득·엔진 무결
   증명). → **RH 정책 확정(사용자 합의)**: 기본은 엑셀4 방식(실측 RH). 60% 는 센서 고장값(유효범위 5~100%
   밖)일 때만 쓰는 임시 방편이며, RH 정상화 시 자동으로 실측 사용(코드 변경 불필요). 고장값 직사용은
   봄가을 +1MW·여름 최대 −2~−6.7MW 왜곡을 유발하므로 배제. **RH 센서(10MBL11CM001) 정비 요청 권고.**
