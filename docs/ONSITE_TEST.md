...
> → `[✅ 누적 반영됨]`. 같은 날짜를 또 반영하면 `[⚠ 이미 반영된 날짜 — 건너뜀]`(중복 방지).
> (GUI 에서는 "이 테스트를 누적에 반영" 체크박스)

> **엑셀1 자동 감지.** `--workbook` 를 생략하면 **exe(또는 현재 폴더)에 둔 엑셀1을 자동으로 찾습니다**
> → 날짜·시간만 입력하면 됩니다(출력에 `엑셀1 자동 감지 : …`). 패턴은 `*Base Load Test*.xlsx`.
> 파일명이 다르면 `--workbook` 로 직접 지정하거나, 환경변수 `WIRYE_WORKBOOK` 에 파일명을 지정하세요.
> GUI 는 시작 시 엑셀1 경로 칸을 자동으로 채웁니다(필요 시 수정 가능).

**확인**: 출력 `적용 대기압(기준: …)`·`신규 취득 CIT/보정값 [상태]`·`누적 건수`. 파일을 Excel에서 열어 온도 Profile 확인.
...
> 실행자가 **반영할 테스트만 `--accumulate`** 로 누적합니다. 단순 확인 실행은 옵션 없이(미반영).
> 누적에 넣고 싶지 않은 결과가 섞이지 않습니다.

1. 매 테스트마다 STEP 4 실행(운영 DB `wirye_commission.db` 계속 사용 → 반영 시 누적).
