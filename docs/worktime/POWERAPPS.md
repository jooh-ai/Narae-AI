...
#### 8-3. 데이터를 어느 사이트에 둘 것인가

지금 목록 5개는 **테스트 팀**에 있다. 30% 판정이 두 팀 합산이므로 운영 데이터는
**두 팀이 모두 접근하는 한 곳**에 있어야 한다. 두 가지 길이 있다.

| 방법 | 내용 | 장단 |
|---|---|---|
| **A. 테스트 팀을 운영 팀으로** | 그 팀에 두 팀 구성원을 초대하고 이름만 바꾼다 | 이전 작업이 **없다**. 팀 이름·기존 채널이 테스트용이면 어색할 수 있다 |
| **B. 실제 채널로 목록 이전** | 두 팀이 함께 있는 채널의 사이트에 CSV 로 목록 5개를 다시 만들고, 앱의 데이터 원본을 갈아끼운다 | 자리는 깔끔하지만 **데이터 원본 교체가 위험하다** (아래) |

**B 를 고를 때의 순서 — 이 순서를 지켜야 수식이 안 깨진다.**
