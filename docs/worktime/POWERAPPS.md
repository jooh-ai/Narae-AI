...
> **`공휴일` 의 날짜 열이 텍스트로 들어가면 안 된다.** 앱이 `LookUp(공휴일, 날짜 = d)` 로
> 비교하므로 날짜 타입이어야 한다. 가져오기 화면에서 반드시 확인하고, 놓쳤으면
> 열 설정에서 바꾸거나 목록을 다시 만든다.

### 2-2. `근태기록` 목록 직접 만들기

1. 채널 → `+` → **Lists** → `목록 만들기` → **`빈 목록`** → 이름 `근태기록`
2. 열 머리글 `제목` → `열 설정` → `이름 바꾸기` → **`키`**
3. `+ 열 추가` 로 §1-3 표의 나머지 14개를 만든다. 타입만 맞추면 된다
   - **날짜**: `근무일` — 만들 때 **`시간 포함` 을 끈다**
   - **숫자**: `실근로` · `휴가` · `충족` · `OT시간` · `주` (실근로·휴가·OT시간은 소수 2자리)
   - **텍스트**: `구성원` · `유형` · `출근` · `퇴근` · `조합코드` · `구분` · `OT시작` · `OT종료` · `석식`
   - **여러 줄 텍스트**: `메모`

> 이 목록은 비어 있는 채로 시작한다. **기록이 없는 날 = 8시간 근무**이므로
> 미리 채워 넣을 것이 없다.
