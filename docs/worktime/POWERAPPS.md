...
화면 다섯 개가 다 검증됐으면 운영으로 넘기기 전에 이것부터 한다.

#### 8-1. 테스트 기록 정리

SharePoint `근태기록` 목록에서 **5행을 지운다**. 앱의 `galSaved` 휴지통으로 지워도 된다.

| 키 | 내용 |
|---|---|
| `2026-09-10_고재민` | 3단계 저장 테스트 |
| `2026-09-15_김동환` · `_정병철` · `_박상호` · `_최규석` | §3-6 판정 테스트 |

지우고 나면 미달일 0 · 전원 160h 로 돌아간다. **`근태기록` 은 비어 있는 채로 운영을
시작한다** — 기록 없는 날 = 8시간 근무이므로 미리 채울 것이 없다.

#### 8-2. 반드시 채워야 할 데이터

| 목록 | 할 일 | 안 하면 |
|---|---|---|
| **`월설정`** | `sp_월설정.csv` 를 다시 가져와 **2026-09 ~ 2027-12 16개월** 을 만들고, 달마다 `OT기본` 을 담당자 확인 값으로 채운다 | **그 달 행이 없으면 `gvOtBase` 가 0** 이 되어 OT 잔여가 전부 음수로 나온다 |
| `공휴일` | 2027년 이후를 추가한다. 음력(설날 · 추석 · 부처님오신날)과 주말과 겹치는 대체공휴일은 해마다 확인해야 한다 | 소정근로일이 과다 계산되어 총량이 어긋난다 |
| `명부` | `사용자` 열(사용자 또는 그룹)을 추가하고 각자 계정을 넣는다 | 본인 자동 선택이 안 되고, 매번 이름을 골라야 한다 |
| 앱 설정 | `설정 → 일반 → 데이터 행 제한` 을 **2000** 으로 | 기록이 500행을 넘는 순간 집계가 **조용히** 틀린다 |

`사용자` 열을 채운 뒤에는 `App.OnStart` 의 「한 번만」 블록에 한 줄을 더하고,
`ddMe.Default` 를 `LookUp(Distinct(명부, Title), Value = Coalesce(gvPickWho, gvMe))`
로 바꾸면 본인이 자동 선택된다.
...
#### 8-3. 먼저 테스트 운영, 이전은 나중에

**앱을 Teams 탭에 붙이는 것과 데이터가 어느 사이트에 있는가는 별개다.** 앱은 어느 팀의
어느 채널에든 탭으로 추가할 수 있고, 데이터는 테스트 사이트의 목록에 그대로 두어도 된다.
두 팀 구성원이 그 사이트에 접근할 수만 있으면 운영이 된다 — 팀에 초대하면 끝이고,
팀 이름이 어색하면 **이름만 바꿔도** SharePoint 사이트 주소는 그대로라 앱이 안 깨진다.

**테스트 운영 체크리스트** — 정식 이전 전에 이 다섯 가지를 겪어보면 충분하다.

| 확인할 것 | 왜 |
|---|---|
| 두 사람이 **동시에** 다른 날짜를 저장 | 요건 13(전원 동시 수정)의 실제 검증 |
| 한 사람이 저장한 것을 **다른 사람 화면에서** 보기 | `scrGrid` 를 나갔다 들어오면 갱신된다 |
| 휴대폰 Teams 앱에서 열기 | 좌표가 PC 기준이라 어떻게 보이는지 확인 |
| 실제 미달일을 만들어 **조율 후보** 써보기 | 이 도구의 본래 목적 |
| **월이 바뀔 때**(10/1) 동작 | `월설정` 에 그 달 행이 있어야 한다 |

#### 8-4. 나중에 옮길 때 — 30~60분

| 항목 | 난이도 | 비고 |
|---|---|---|
| `조합표` · `명부` · `공휴일` · `월설정` | 쉬움 (10분) | CSV 4개를 그대로 다시 가져온다 |
| `근태기록` 목록 껍데기 | 쉬움 (5분) | `sp_근태기록_시작.csv` 로 |
| **`근태기록` 의 실제 데이터** | **유일한 부담** | 목록 → `Excel로 내보내기` → 새 목록에 가져오기. 날짜·숫자 타입을 다시 맞춰야 한다 |
| 앱 데이터 원본 교체 | 보통 (15분) | 아래 순서 |
| 다섯 화면 확인표 재검증 | 20분 | §3-3 · §3-6 · §3-7 · §3-8 · §3-10 |

> **테스트 기간의 `근태기록` 을 버릴 수 있으면 이전이 10분짜리 일이 된다.** 기록 없는
> 날 = 8시간 근무가 기본이므로, 비우는 것은 데이터 손실이 아니라 정상 상태로 되돌리는
> 것이다. **정식 운영 시작일에 `근태기록` 을 비우고 시작**하는 것을 권한다.

#### 8-5. 데이터 원본 교체 순서

**이 순서를 지켜야 수식이 안 깨진다.**

1. 앱을 **저장**한다 (되돌릴 버전을 만들어 둔다)
2. 새 사이트에 CSV 4개 + `근태기록` 으로 목록 5개를 만든다. **목록 이름을 똑같이** 짓는다
3. 앱 편집기 `데이터` 에서 **기존 5개를 먼저 제거**한다 → 앱 전체가 오류가 된다 (정상)
4. 새 사이트의 목록 5개를 추가한다 → 이름이 같으므로 수식이 그대로 살아난다
5. `OnStart 실행` 후 다섯 화면의 확인표를 다시 돌린다

> **먼저 추가하고 나중에 제거하면 안 된다.** 이름이 겹쳐 `조합표_1` 로 붙고 수식을
> 전부 고쳐야 한다. **반드시 제거 → 추가 순서**다.

## 4-10. 디자인 규칙 — 카드 · 구분 · 위계

기능이 다 검증된 뒤 한 번에 적용한다. 원칙은 세 가지다.

1. **화면 배경은 회색, 내용은 흰 카드 위에.** 카드 경계가 곧 「한 덩어리」의 경계다
2. **글자 크기로 위계를 만든다.** 값(20) > 제목(13) > 본문(11) > 보조(9)
3. **색은 판정에만 쓴다.** 장식으로 색을 쓰면 빨강·주황의 의미가 흐려진다

### 토큰

| 용도 | 값 | Power Fx |
|---|---|---|
| 화면 배경 | `#f6f7f9` | `RGBA(246, 247, 249, 1)` |
| 카드 배경 | `#ffffff` | `RGBA(255, 255, 255, 1)` |
| 테두리 · 구분선 | `#e3e6ea` | `RGBA(227, 230, 234, 1)` |
| 본문 글자 | `#16181d` | `RGBA(22, 24, 29, 1)` |
| 보조 글자 | `#6b7280` | `RGBA(107, 114, 128, 1)` |
| 강조 (선택 탭 · 헤더) | `#1c4e80` / `#e8eef6` | `RGBA(28, 78, 128, 1)` / `RGBA(232, 238, 246, 1)` |

판정 색(초록 · 주황 · 빨강 · 회색)은 §4-9 그대로 쓴다.

### 카드 만드는 법

Power Apps 에는 「카드」가 없으므로 **사각형 도형을 뒤에 깔고** 그 위에 컨트롤을 올린다.

`삽입 → 도형 → 사각형` → 이름을 `rect...` 로 → **오른쪽 클릭 → 순서 → 맨 뒤로 보내기**

| 속성 | 값 |
|---|---|
| `Fill` | `RGBA(255, 255, 255, 1)` |
| `BorderColor` | `RGBA(227, 230, 234, 1)` |
| `BorderThickness` | `1` |
| `RadiusTopLeft` · `RadiusTopRight` · `RadiusBottomLeft` · `RadiusBottomRight` | `8` |

카드 위에 놓이는 갤러리는 `Fill` 을 **투명**(`RGBA(0, 0, 0, 0)`)으로 둔다. 그래야 카드의
흰 배경과 둥근 모서리가 보인다.

> **도형은 단추처럼 동작한다 — `HoverFill` 을 반드시 끈다.** 기본 `HoverFill` 이 파란
> 계열이라, 카드 위에 마우스가 가면 **카드 전체가 연보라색으로 변한다.** 도형마다 네
> 속성을 넣어 평소 색과 같게 만든다.
>
> | 속성 | 값 |
> |---|---|
...
>
> `Self` 를 쓰면 나중에 `Fill` 을 바꿔도 따라온다. 도형을 복제(`Ctrl+C` → `Ctrl+V`)하면
> 이 설정도 같이 복사되므로, **하나를 완성한 뒤 복제**하는 편이 빠르다.

> **오른쪽 속성 창의 입력 상자에는 따옴표를 넣지 않는다.** 거기는 글자 그대로
> 들어가서 `"메모 (선택)"` 처럼 따옴표까지 화면에 보인다. 따옴표는 **수식 바(`fx`)에
> 넣을 때만** 쓴다.

### 탭 바

선택 탭을 배경색이 아니라 **밑줄**로 표시한다. 배경색은 판정 색과 헷갈린다.

| 컨트롤 | 속성 | 값 |
|---|---|---|
| `galTabs` | `Fill` | `RGBA(255, 255, 255, 1)` |
| | `TemplateFill` | `RGBA(0, 0, 0, 0)` |
| `lblTab` | `Color` | `If(ThisItem.키 = gvTab, RGBA(28,78,128,1), RGBA(107,114,128,1))` |
| | `FontWeight` | `If(ThisItem.키 = gvTab, FontWeight.Semibold, FontWeight.Normal)` |
| `rectTabMark` (갤러리 **안** 사각형) | `X`·`Y`·`W`·`H` | 20 · 37 · 130 · 3 |
| | `Fill` | `RGBA(28, 78, 128, 1)` |
| | `Visible` | `ThisItem.키 = gvTab` |
| `rectTabLine` (화면 위 사각형) | `X`·`Y`·`W`·`H` | 0 · 39 · 1366 · 1 |
| | `Fill` | `RGBA(227, 230, 234, 1)` |

### 빈 상태는 반드시 문구를 넣는다

갤러리에 행이 0개면 Power Apps 는 아무것도 그리지 않아 **고장 난 것처럼 보인다.**
빈 상태 레이블을 겹쳐 두고 `Visible` 로 켠다.

```powerfx
CountRows(galSaved.AllItems) = 0
```

| 갤러리 | 빈 상태 문구 |
|---|---|
| `galSaved` | `"저장한 기록이 여기에 표시됩니다"` |
| `galCandidate` | `"달력에서 날짜를 고르면 조율 후보가 표시됩니다"` |
