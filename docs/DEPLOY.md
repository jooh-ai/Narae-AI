...
**적용 대기압** — 예보 파일만 올리면 자동으로 정해진다: **3~7일차(테스트일 +2 ~ +6)
중위 대기압의 평균 − 8 mbar**(성남비행장↔발전소 위치보정). 엑셀3 `온도 Profile`!M2 =
`=AVERAGE(Y8:Y12)-8` 과 같은 규칙이라 고를 수 없다 — 적용일 선택 칸은 2026-08-25 에
제거했다(종전 7일 전체 평균은 M2 표시값과 어긋났다). 대기압 민감도는 약 0.4 MW/mbar 다.
...
방법은 7가지다: **GP·RBF(기본)** · GP 4종(Matérn 5/2 / Matérn 3/2 / 지수 /
Rational Quadratic) · 커널회귀 · 구간평균. GP 는 **커널이 곡선의 성격**을 정하고
하이퍼파라미터는 커널마다 주변우도로 자동 적합한다 — 사람이 만질 값은 커널 하나다.

기본값이 **GP·RBF** 다(2026-08-25 변경). 누적 36건 LOOCV 예측오차가
GP·RBF 1.294 < 구간평균 1.515 < 종전 일괄 3.833 MW 이다. 종전 기본은
엑셀4 방식(구간평균)이었다.
