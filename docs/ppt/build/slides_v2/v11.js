...
'use strict';
const KEEP = ['학습·채점·재선정 코드', '검증 절차와 자동 가드', '신고값 산출과 화면'];
const SWAP = ['설비별 이론 출력 계산식', '성능 곡선과 설비 상수'];
const PLAN = [
  ['~ 2026.09', '위례 정착',        '누적 회차로 운영 중'],
  ['2026.10~12', '절차 문서화',      '검증·가드 기준을 사내 문서로'],
  ['2027 상반기', '동일 구조 1곳 시범', '이론만 교체해 병행 운전'],
  ['2027 하반기', '수평 전개',        '이론 모듈 분리 후 확대'],
];
module.exports = (pptx, T, meta, D) => {
  const { C, G } = T;
  const { d } = T.shell(pptx, { sec: '수평 전개', idx: 7, step: 7 });
  T.title(d, '구조를 둘로 나눈 덕분에,', '*이론만 갈아끼우면* 옮겨집니다');
  T.lead(d, '2층(학습)은 설비 종류와 무관합니다. 사업소마다 다른 것은 _1층(이론)_ 하나입니다.',
         { lines: 1 });
  ...
  T.foot(d, '위례에서 검증한 절차 그대로 — *바꿀 것은 이론 하나*입니다.');
};
