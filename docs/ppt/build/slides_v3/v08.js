...
module.exports = (pptx, T, meta, D) => {
  ...
  d.text('엑셀 4개를 오가며 61개 값을 옮기던 일이, 날짜와 시각을 넣는 일로 바뀌었습니다.',
         { x: 92, y: y1 + 96, w: 516, px: 12.5, lh: 1.35, lines: 2, color: C.body });
  ...
  /* ── 구획 3 · 그 숫자를 어떻게 쟀나 ───────────────────────────── */
  const y3 = d.section(G.L, 336, G.W, 112, 3, '정확도를 어떻게 쟀나',
                       '실제로 신고한 기록이 아니라 되돌려 본 채점입니다');
  [['한 회를 가린다', '시험 ' + D.n + '회 가운데 한 회를 빼 둡니다.'],
   ['나머지로 맞혀 본다', '가린 회차의 온도만 주고 보정값을 예측하게 합니다.'],
  ...
  d.text('종전 방식도 같은 회차에 「늘 같은 값 하나」를 더해 똑같이 채점했습니다. ' +
         ...
         '회를 썼습니다 — 한 회차는 그 온도 구간에 시험이 그것뿐이어서 가리면 ' +
         '비교 대상 방식 하나가 답을 못 냅니다.',
         { x: 92, y: y3 + 56, w: 1090, px: 10, lh: 1.35, lines: 2, color: C.dim2 });

  /* ── 구획 4 · 돈으로 보면 ─────────────────────────────────────── */
  const y4 = d.section(G.L, 460, G.W, 180, 4, '돈으로 보면', '용량요금 기준');
  ...
  T.foot(d, '일은 줄고 숫자는 맞았습니다. 처음에 세운 목표 두 가지입니다.');
};
