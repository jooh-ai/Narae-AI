...
module.exports = (pptx, T, meta, D) => {
  ...
  const NAME = { cit: '외기 온도만', rh: '＋ 상대습도', press: '＋ 대기압',
                 cp_meas: '＋ 복수기 진공도' };
  const WHY = {
    cit: '차이의 *85%* 는 온도 하나로 설명됩니다',
    rh: '0.01 나아졌지만 흔들리는 폭 ±' + near.vs_base.se.toFixed(2) + ' 보다 작습니다',
    press: '제작사 계산식이 이미 쓰는 값입니다 (두 번 반영됩니다)',
  ...
  const tb = d.table(92, y1, COL,
    V.rows.map((r, i) => [
      ...
      { t: WHY[key(r)], px: 11.5 }]),
    { rh: 24, hh: 26, hi: 0 });
  d.text('단위 MW · 작을수록 잘 맞힌 것입니다. 회차마다 ±' +
         near.vs_base.se.toFixed(2) + ' 은 그냥 흔들리는 폭이라, 그보다 작은 차이는 ' +
         '의미가 없습니다. 채점은 한 회를 가리고 나머지로 그 회를 맞혀 보는 방식입니다.',
         { x: 92, y: y1 + tb.h + 10, w: 1090, px: 10.5, lh: 1.3, color: C.dim2 });

  /* ── 구획 2 · 외기 온도가 남은 이유 ──────────────────────────── */
  const y2 = d.section(G.L, 422, 556, 218, 2, '외기 온도가 남은 이유', '');
  [['온도 하나로 대부분이 설명된다',
    '추우면 차이가 커지고, 더우면 작아집니다. 온도만 넣어도 대부분 맞습니다.'],
   ['방향이 분명하다',
    '온도가 오르면 차이가 줄어듭니다 (반대 방향 ' + Math.abs(CZ.rows[0].raw).toFixed(2) +
    '). 네 후보 가운데 가장 뚜렷합니다.'],
   ['미리 알 수 있는 값이다',
    '신고는 영하 20도부터 40도까지 온도마다 해야 합니다. 입력도 온도여야 표가 나옵니다.']]
    .forEach(([k, v], i) => {
      const y = y2 + i * 60;
  ...
  d.text('봄 시험 몇 회가 유별나게 낮았습니다. 설비가 낡아서 그렇다고 보고서까지 썼습니다.',
  ...
  d.text('이 시험은 *출력을 끝까지 올린 상태*에서 해야 합니다. 가스터빈 입구 날개를 ' +
         '더 여는 조작(IGV turn-up)을 하지 않고 측정한 날이었습니다. ' +
         '덜 올린 채로 쟀으니 낮게 나왔습니다.',
         { x: 684, y: y3 + 68, w: 504, px: 12, lh: 1.45, lines: 4, color: C.body });
  d.text('그 보고서는 취소했습니다. 지금은 그런 날의 시험을 도구가 걸러냅니다.',
         { x: 664, y: y3 + 150, w: 524, px: 12, lh: 1.35, color: C.dim2 });

  d.hline(G.L, G.RULE2, G.W, C.rule, 1);
  T.foot(d, '후보를 하나씩 빼 보니 외기 온도만 남았습니다.');
};
