...
module.exports = (pptx, T, meta, D) => {
  ...
  T.lead(d, '사람이 다시 계산하지 않습니다. 시험 결과를 넣으면 곡선이 스스로 다시 배웁니다.',
         { y: 146, lines: 1 });
  ...
  STEP.forEach((s, i) => {
    ...
    d.text(String(i + 1), { x, y: 232, w: 24, px: 15, lh: 1.2, mono: true, bold: true,
                            color: C.brass });
    d.text(s, { x: x + 28, y: 230, w: 200, px: 15.5, lh: 1.3, color: C.ink });
    if (i < 3) d.arrow(x + 240, 230);
  });

  /* 구획 2 — 도구 화면 */
  d.section(G.L, 306, 700, 318, 2, '도구 화면', '누적 ' + D.n + '회');
  d.imgFit(A.toolWin, 92, 344, 660, 232);
  d.text('날짜와 시각을 넣고 실행하면 61개 온도의 신고값이 한 번에 나옵니다.',
         { x: 92, y: 586, w: 660, px: 12, lh: 1.3, color: C.dim2 });

  /* 구획 3 — 걸러내는 장치 */
  d.section(788, 306, 420, 318, 3, '잘못된 회차를 걸러냅니다', '');
  [['IGV 를 올리지 않은 시험', '누적에 넣지 않습니다',
    '앞에서 결론을 뒤집게 만든 그 회차입니다.'],
   ['시험한 범위를 벗어난 온도', '끝값을 그대로 씁니다',
    '배우지 않은 구간을 멋대로 늘려 쓰지 않습니다.'],
