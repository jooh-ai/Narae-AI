...
module.exports = (pptx, T, meta, D) => {
  ...
  T.title(d, '우리가 쓰는 회귀 모델', null, { px: 33 });
  T.lead(d, '모델은 시험 결과 점 사이로 곡선을 하나 긋습니다. 곡선을 긋는 방법이 모델마다 다릅니다.',
         { y: 134, lines: 1 });
  ...
  [['GP 는 곡선을 하나만 고르지 않습니다',
    '점을 지나는 곡선은 여러 개 있습니다. GP 는 그 곡선들의 가운데를 답으로 냅니다.'],
   ['값과 함께 범위도 나옵니다',
    '점선 두 줄 사이가 90% 범위입니다. 다른 방법은 값만 주고 범위는 주지 않습니다.'],
   ['커널이 곡선 모양을 정합니다',
    '곡선을 얼마나 부드럽게 그을지 정하는 설정입니다. 아래에서 셋을 비교했습니다.']]
    .forEach(([k, v], i) => {
      const y = y1 + i * 58;
  ...
  /* ── 구획 2 · 커널을 바꾸면 ───────────────────────────────────── */
  const y2 = d.section(G.L, 398, G.W, 242, 2, '커널 세 가지 비교',
  ...
  const CARD = [
    ['rbf', 'GP · RBF', '우리가 쓰는 것',
     'RBF 는 옆 온도와 비슷하게 이어 줍니다. 그래서 곡선이 가장 부드럽습니다.',
     '시험이 없는 온도도 채워 줍니다',
     '갑자기 꺾이는 곳은 놓칩니다', false],
    ['exp', 'GP · 지수', '',
     '지수는 바로 옆 점만 보고 따라갑니다. 그래서 곡선이 구불거립니다.',
     '갑자기 꺾이는 곳도 잡습니다',
     '흔들림까지 따라가 RBF 보다 ' + rr + '배 구불거립니다', true],
    ['curve', '커널회귀', 'GP 가 아닌 방법',
     '커널회귀는 가까운 온도의 시험값을 평균 냅니다. 가까운 값을 더 크게 봅니다.',
     '계산이 단순해서 이해하기 쉽습니다',
  ...
  T.foot(d, '세 곡선은 눈으로 봐서 고를 만큼 다르지 않았습니다. 그래서 성적으로 골랐습니다.');
};
