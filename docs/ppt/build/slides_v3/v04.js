...
module.exports = (pptx, T, meta, D) => {
  ...
  T.lead(d, '절차가 번거로웠고, 더하는 값은 늘 하나였습니다. 그래서 신고 숫자가 실제와 어긋났습니다.',
         { y: 134, lines: 1 });
  ...
  let y2 = d.section(644, 174, 564, 116, 2, '개선 목표', '목표 두 가지');
  /* 2026-09-17 회신: "온도마다 다르게 — '시험 실적을 쌓아 온도별 값으로' 라고
     텍스트가 있는데 목적어가 없음. PPT 전반적으로 검토해서 주어나 목적어가
     없는 텍스트는 문장이 되도록 수정." 서술을 문장으로 고쳤다. */
  [['하나로 합치기', '엑셀 파일 4개로 하던 일을 도구 하나로 합칩니다.'],
   ['온도마다 다르게', '시험 실적을 쌓아서 더하는 값을 온도마다 다르게 만듭니다.']]
    .forEach(([k, v], i) => {
    const x = 664 + i * 272;
  ...
  d.text('겨울에는 낼 수 있는데 적게 신고하고, 여름에는 못 낼 만큼 많이 신고하게 됩니다.',
