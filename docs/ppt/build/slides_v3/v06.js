...
module.exports = (pptx, T, meta, D) => {
  ...
  /* 구획 1 — 두 기둥 */
  d.section(G.L, 186, 400, 232, 1, '바꾼 것 두 가지', '');
  ...
  /* 안 바꾼 것도 적는다. 바꾼 것만 적으면 "계산까지 새로 만들었나" 로
     읽힌다 — 실제로 질문을 받은 대목이다. */
  d.text('바꾸지 않은 것', { x: 92, y: 380, w: 360, px: 11, lh: 1.2, mono: true,
                              bold: true, color: C.dim2, cs: 1.2 });
  d.text('제작사 계산식은 손대지 않았습니다. 계측값을 받아오는 방식도 기존 엑셀이 ' +
         '쓰던 그대로입니다.',
         { x: 92, y: 398, w: 360, px: 12.5, lh: 1.4, lines: 2, color: C.dim });
  ...
  /* 구획 3 — 겪은 일 */
  d.section(G.L, 430, G.W, 194, 3, '방법을 고른 과정',
            '막대가 짧을수록 잘 맞힌 것 · 단위 MW');
  d.text('어느 방법이 맞을지 우리가 정할 수 없었습니다. 그래서 후보 일곱 가지를 ' +
  ...
  d.text('* 후보 7가지 = 구간 평균 · 거리가중 평균 · 가우시안 프로세스(GP) 커널 5종. ' +
         ...
         { x: 92, y: 614, w: 1090, px: 10, lh: 1.35, color: C.dim2 });
