...
아이폰에서 촬영한 HEIF/HEIC 이미지를 JPG 또는 PNG로 변환하는 도구입니다.
설치가 필요 없는 **웹(HTML)** 버전과, 파이썬 기반의 **CLI / GUI** 버전을 제공합니다.

## 웹(HTML) 버전 — 설치 불필요, 권장

파이썬이 없어도 됩니다. `heif_converter.html` 파일을 **Chrome 또는 Edge**로 열기만 하면
됩니다. 사진은 인터넷으로 전송되지 않고 100% 내 컴퓨터(브라우저) 안에서만 변환됩니다.

1. `heif_converter.html`을 더블클릭해 브라우저로 엽니다.
   (같은 위치에 있는 `vendor/` 폴더가 함께 있어야 합니다 — HEIC 디코딩 라이브러리가 들어 있습니다.)
2. **입력 폴더**를 선택합니다 (변환할 HEIC 사진이 든 폴더).
3. **출력 폴더**를 선택합니다 (변환 파일을 저장할 새 폴더).
4. 형식(JPG/PNG)·품질·하위 폴더 포함·덮어쓰기를 지정하고 **변환 시작**을 누릅니다.
5. 원본 폴더 구조를 유지한 채 출력 폴더에 저장됩니다.

> - 폴더째 자동 저장은 Chrome/Edge의 File System Access API를 사용합니다.
> - Firefox·Safari에서는 폴더 직접 저장이 안 되며, 파일을 선택해 변환한 뒤
>   다운로드 링크로 내려받는 방식으로 자동 전환됩니다.
> - 인터넷 연결 없이 동작합니다(라이브러리를 `vendor/`에 내장).

---

## 파이썬 버전

아래는 파이썬이 설치된 환경에서 쓰는 CLI / GUI 버전입니다.
