...
파이썬이 없어도 됩니다. `heif_converter.html` **파일 하나**를 **Chrome 또는 Edge**로 열기만
하면 됩니다. HEIC 디코딩 라이브러리가 파일 안에 내장돼 있어 다른 파일이 필요 없고,
사진은 인터넷으로 전송되지 않고 100% 내 컴퓨터(브라우저) 안에서만 변환됩니다.
...
> - **파일 하나로 완결**됩니다. 라이브러리가 HTML에 내장돼 있어 `vendor/` 폴더나
>   인터넷 연결이 없어도 동작합니다.
